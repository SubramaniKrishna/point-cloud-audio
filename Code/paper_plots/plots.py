...
for F in dict_baseline["data"].keys():
    pyp.plot(list_N_baseline,dict_baseline["data"][F],'.-')
...
for F in dict_settransformer["data"].keys():
    pyp.plot(list_N_settransformer,dict_settransformer["data"][F],'.-')
...
pyp.plot(list_tK_b,topK_acc_b,label = "CNN_temp Top")
pyp.errorbar(list_rK_b,randK_acc_b,yerr = randK_var_b, label = "CNN_temp Rand")
pyp.legend(loc = 'best')
pyp.grid(True)
